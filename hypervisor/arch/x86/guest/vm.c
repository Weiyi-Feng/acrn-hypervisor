...
uint16_t get_vmid_by_name(const char *name)
{
	...
	for (uint16_t idx = 0U; idx < CONFIG_MAX_VM_NUM; idx++) {
		if (vm_has_matched_name(idx, name)) {
			matched_cnt++;
...
bool is_valid_vm_name(char *name)
{
	return (get_vmid_by_name(name) != ACRN_INVALID_VMID);
}
...
#endif
	if (!is_valid_vm_name(vm_config->name)) {
		pr_err("Invalid VM name:%s\r\n", vm_config->name);
		err = -1;
	}
